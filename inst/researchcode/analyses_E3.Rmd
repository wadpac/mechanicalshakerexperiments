...
save(raw_spectra, file = paste0(datadir, "E3_frequency_spectra.RData"))
rm(raw.spectra_low, SD, sampling_frequency)

#load(paste0(datadir, "E3_frequency_spectra.RData"))
```

## Mean frequency spectra

```{r Plot mean frequency spectrum low sampling frequency, echo=FALSE}
plot(raw_spectra$mean_spectrum$x, raw_spectra$mean_spectrum$y, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l", main = "Mean frequency spectrum low sampling frequency and constant dynamic range")
rm(mean.freqspec_low)
```

## Derive frequency bins
```{r Derive frequency bins low sampling frequency, echo=FALSE}
peaks_low <- derivePeaks(raw_spectra$mean_spectrum, 100, XLIM, plot = TRUE)
...
#cat("\nSaving data...")
save(comparison_values, file = paste0(datadir, "E3_comparison_values.RData")) 
...
#cat("\nSaving data...")
save(df_long, file = paste0(datadir, "E3_comparison_values_long.RData"))
rm(peaks_low, index_low)

#load(datadir, "E3_comparison_values_long.RData")
```
...
#### Interaction effect
```{r Calculate contrasts interaction effect meanPSD for low sampling frequency experiment, echo=FALSE}
...
ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_E3_meanPSD.png"), bxp_psd, width = 10, height = 8, dpi = 900) #saves g
ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_E3_domFreq.png"), bxp_domFreq, width = 10, height = 8, dpi = 900) #saves g
```
...
save(correlation_matrices, file = paste0(datadir, "E3_cross_correlations_overall.RData"))

correlations_perShakerCondition <- computeCrossCorrelationsPerShakerCondition(data_low$data) 
save(correlations_perShakerCondition, file = paste0(datadir, "E3_cross_correlations_perShakerCondition.RData"))
```
...
names(table_between) <- names(correlations_perShakerCondition$low)
```
...
```{r Line graph low sampling frequency, echo=FALSE}
...
ggplot2::ggsave(file=paste0(datadir, "plots/linegraph_E3_crosscorrelations.png"), line_plot, width = 10, height = 8, dpi = 900) #saves g
```
