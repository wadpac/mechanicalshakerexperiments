...
#cat("\nSaving data...")
save(comparison_values_sdCorrected, file = paste0(datadir, "E5_comparison_values_sdCorrected.RData")) 
...
# Wide format: for both outcomes domFreq and meanPSD
load(paste0(datadir, "E5_comparison_values_sdCorrected.RData"))
df_wide <- data.frame()
...
#cat("\nSaving data...")
save(df_long, file = paste0(datadir, "E5_comparison_values_long_sd_corrected.RData"))
rm(peaks_sdCorrected)

#load(paste0(datadir, "E5_comparison_values_long_sd_corrected.RData"))
```
...
# Evaluate if adding a random intercept for frequency bin is necessary
meanPSD_interaction_id_bin <- lme4::lmer(meanPSD ~ bin*brand + (1 | serialnumber) + (1 | bin), REML = TRUE, data = df_long) 
anova(meanPSD_interaction_id_sd_corrected, meanPSD_interaction_id_bin, refit = FALSE) # This is not necessary, so use simplest model
rm(meanPSD_interaction_id_bin)
```
...
ggpubr::ggqqplot(df_long, "meanPSD", facet.by = "bin", title = "QQ-plot mean power spectral density")
residuals_meanPSD_sd <- resid(meanPSD_interaction_id_sd_corrected)
#summary(residuals_meanPSD)
hist(residuals_meanPSD)
rm(residuals_meanPSD)
```
...
require(lmerTest)
test.meanPSD <- as(meanPSD_interaction_id_sd_corrected, "merModLmerTest")
print(summary(test.meanPSD, ddf="Kenward-Roger"), correlation = FALSE)
...
maineffect_bin <- emmeans::emmeans(meanPSD_interaction_id_sd_corrected, comparison="pairwise", 
                                         specs= ~ bin, adjust="sidak", data = df_long)
maineffect_bin
emmeans::contrast(maineffect_bin, "pairwise")
rm(maineffect_bin)
...
# Evaluate if adding a random intercept for frequency bin is necessary
domFreq_interaction_id_bin <- lme4::lmer(domFreq ~ bin*brand + (1 | serialnumber) + (1 | bin), REML = TRUE, data = df_long)
anova(domFreq_interaction_id_sd_corrected, domFreq_interaction_id_bin, refit = FALSE) # This is not necessary, so use simplest model
rm(domFreq_interaction_id_bin)
```
...
require(lmerTest)
test.domFreq <- as(domFreq_interaction_id_sd_corrected, "merModLmerTest")
print(summary(test.domFreq, ddf="Kenward-Roger"), correlation = FALSE)
...
maineffect_brand <- emmeans::emmeans(domFreq_interaction_id_sd_corrected, comparison="pairwise", 
                                         specs= ~ brand, adjust="sidak", data = df_long)
maineffect_brand
emmeans::contrast(maineffect_brand, "pairwise")
rm(maineffect_brand)
...
interactioneffect <- emmeans::emmeans(domFreq_interaction_id_sd_corrected, comparison="pairwise", 
                                         specs= ~ bin * brand, adjust="sidak", data = df_long)
...
bxp_psd <- createBoxplot(df_long, bins_sdCorrected, outcome = "meanPSD", group = "brand", sampling_frequency = "E5") 
bxp_domFreq <- createBoxplot(df_long, bins_sdCorrected, outcome = "domFreq", group = "brand", sampling_frequency = "E5")

ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_E5_meanPSD_sd_corrected.png"), bxp_psd, width = 10, height = 8, dpi = 900) #saves g
ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_E5_domFreq_sd_corrected.png"), bxp_domFreq, width = 10, height = 8, dpi = 900) #saves g
```
...
save(correlation_matrices, file = paste0(datadir, "E5_cross_correlations_overall_sd_corrected.RData"))
...
save(correlations_perShakerCondition, file = paste0(datadir, "E5_cross_correlations_perShakerCondition_sd_corrected.RData"))

#load(paste0(datadir, "E5_cross_correlations_overall_sd_corrected.RData"))
#load(paste0(datadir, "E5_cross_correlations_perShakerCondition_sd_corrected.RData"))
```
...
ggplot2::ggsave(file=paste0(datadir, "plots/linegraph_E5_crosscorrelations_sd_corrected.png"), line_plot, width = 10, height = 8, dpi = 900) #saves g
```
