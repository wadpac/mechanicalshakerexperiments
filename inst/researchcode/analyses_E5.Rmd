...
#load(datadir, "E5_comparison_values_long_sd.RData")
```
...
#load(datadir, "E5_comparison_values_long_sd_corrected.RData")
```
...
bins_VM <- deriveBins(peaks_VM, raw_spectra$mean_spectrumVM, XLIM = c(0, 5), plot = TRUE) 
```
### Frequency spectra plots
...
#cat("\nSaving data...")
save(comparison_values_VM, file = paste0(datadir, "E4_comparison_values_VM.RData")) 
...
domFreq_long <- domFreq_ms_wide %>%
  gather(key = "bin", value = "domFreq", "1", "2") %>%
  rstatix::convert_as_factor(serialnumber, bin, brand, sampling_frequency, dynamic_range)
meanPSD_long <- meanPSD_ms_wide %>%
  gather(key = "bin", value = "meanPSD", "1", "2") %>%
  rstatix::convert_as_factor(serialnumber, bin, brand, sampling_frequency, dynamic_range)
df_long <- merge(domFreq_long, meanPSD_long) # Join both data.frame objects to derive a single dataset
rm(domFreq_long, domFreq_ms_wide, meanPSD_long, meanPSD_ms_wide)
df_long <- df_long[order(df_long$serialnumber),]
...
#cat("\nSaving data...")
save(df_long, file = paste0(datadir, "E5_comparison_values_long_VM.RData"))
rm(peaks_VM)

#load(datadir, "E5_comparison_values_long_VM.RData")
```
