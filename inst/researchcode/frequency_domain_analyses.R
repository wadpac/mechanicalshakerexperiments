...
specifications$serial_number[deviations] #42 (AP672490), 51 (AP473254), 55 (AP672490), 64 (AP473254), 78 (6011406), and 93 (6011406)
...
mean(data$aP_490$HA)
sd(data$aP_490$HA)
#AP473254: in both experiments values around 0.3 (M = 0.32503, SD = 0.00628132) was measured during all shaking frequencies
mean(data$aP_254$HA)
sd(data$aP_254$HA)
...
rm(spectrum, i, filename)

# COMPUTE: dominant frequency and mean power spectral density for HA 
...
meanPSD_intercept_id_low_HA <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, method="ML", data = df_low)
getOutputMM(meanPSD_intercept_id_low_HA)
# Dominant frequency
df_low$brand <- relevel(df_low$brand, ref = 5) 
domFreq_intercept_id_low_HA <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_low)
getOutputMM(domFreq_intercept_id_low_HA)
