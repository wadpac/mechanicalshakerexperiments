---
title: 'Analyses Experiment 2: High sampling rate constant dynamic range'
...
filename_experiment = paste0(datadir, "E2_hrcr.RData") 
...
# Select data with high sampling rate (100 Hz) only
index <- which(data$specifications$experiment == "ms_hrcr" & round(as.numeric(data$specifications$sampling_rate)) == "100")
...
# No data recorded for Ax_287
removeAx <- which(names(data_high$data) %in% c("Ax_287"))
data_high$data[removeAx] <- NULL
data_high$specifications <- data_high$specifications[-removeAx,]

setdiff(data$specifications$label, data_high$specifications$label)

outputdir <- "/Users/annelindelettink/Documents/Work MacBook Pro Annelinde/Mechanical Shaker Machine/analyses/E2_hrcr/"
```

# Plot selected axis shaking direction 
```{r plot signals shaking direction hrcr, echo=FALSE}
plot_list <- list()
...
```{r Derive frequency spectra high sampling rate experiment, echo=FALSE}
...
for (file in 1:length(data_high$data)) { # for all accelerometer files
  ...
  rawSpectrum <- deriveSpectrum(SD, sampling_rate, file_name = names(data_high$data)[[file]], datadir = outputdir)
...
save(raw_spectra, file = paste0(outputdir, "E2_frequency_spectra.RData"))
rm(raw.spectra_high, SD, sampling_rate)

#load(paste0(outputdir, "E2_frequency_spectra.RData"))
```

## Mean frequency spectra

```{r Plot mean frequency spectrum high sampling rate, echo=FALSE}
plot(raw_spectra$mean_spectrum$x, raw_spectra$mean_spectrum$y, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l", main = "Mean frequency spectrum high sampling rate and constant dynamic range")
...
```{r Overlay all spectra high sampling rate experiment, echo=FALSE}
...
```{r Plot spectra high sampling rate experiment, echo=FALSE}
...
#cat("\nSaving data...")
save(comparison_values, file = paste0(outputdir, "E2_comparison_values.RData")) 
...
#cat("\nSaving data...")
save(df_long, file = paste0(outputdir, "E2_comparison_values_long.RData"))
rm(peaks_high, index)

#load(outputdir, "E2_comparison_values_long.RData")
```
...
```{r Fit model meanPSD for high sampling rate experiment, echo=FALSE}
...
```{r Mean PSD Model assumptions high sampling rate experiment, echo=FALSE}
...
### Model summary
```{r Model summary meanPSD high sampling rate experiment, echo=FALSE}
...
#### Bin
```{r Calculate contrasts bin meanPSD for high sampling rate experiment, echo=FALSE}
...
#### Brand
```{r Calculate contrasts brand meanPSD for high sampling rate experiment, echo=FALSE}
...
#### Interaction effect
```{r Calculate contrasts interaction effect meanPSD for high sampling rate experiment, echo=FALSE}
...
```{r Fit model domFreq for high sampling rate experiment, echo=FALSE}
...
### Model assumption: normal distribution of residuals
```{r Dominant frequency Model assumptions high sampling rate experiment, echo=FALSE}
...
```
### Model summary
```{r Model summary domFreq high sampling rate experiment, echo=FALSE}
...
### Pairwise comparisons
#### Bin
```{r Calculate contrasts bin domFreq for high sampling rate experiment, echo=FALSE}
...
#### Brand
```{r Calculate contrasts brand domFreq for low sampling rate experiment, echo=FALSE}
...
#### Interaction effect
```{r Calculate contrasts interaction effect domFreq for high sampling rate experiment, echo=FALSE}
...
ggplot2::ggsave(file=paste0(outputdir, "plots/boxplot_E2_meanPSD.png"), bxp_psd, width = 10, height = 8, dpi = 900) #saves g
ggplot2::ggsave(file=paste0(outputdir, "plots/boxplot_E2_domFreq.png"), bxp_domFreq, width = 10, height = 8, dpi = 900) #saves g
```
...
save(correlation_matrices, file = paste0(outputdir, "E2_cross_correlations_overall.RData"))

correlations_perShakerCondition <- computeCrossCorrelationsPerShakerCondition(data_high$data) 
save(correlations_perShakerCondition, file = paste0(outputdir, "E2_cross_correlations_perShakerCondition.RData"))

#load(paste0(outputdir, "E2_cross_correlations_perShakerCondition.RData"))
```

### Overall heatmap
```{r Heatmap high sampling rate experiment, echo=FALSE, fig.width=14, fig.height=14}
...
```{r Heatmap high sampling rate experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=15}
fullmatrices <- correlations_perShakerCondition

heatmaps <- list()
for (condition in 1:length(fullmatrices)){
    png(height=1800, width=1800, file=paste0(outputdir, paste0("plots/heatmaps/heatmap_E2_", condition, ".png")))
...
```{r Calculate average maximum cross lagged correlation coefficients high sampling rate experiment per shaker condition, include=FALSE}
...
```{r Between brand comparison high sampling rate experiment, echo=FALSE}
table_between <- list()
...
save(table_between, file = paste0(outputdir, "E2_line_graph_data.RData"))
```
...
line_plot <- ggplot2::ggplot(data = overview_tab_between, ggplot2::aes(x = as.factor(shaker_frequency), y = as.numeric(correlations), group = as.factor(comparison))) +
  ...
  ggplot2::scale_color_manual(values = c("#440154FF", "#31688EFF", "#35B779FF")) +
...
ggplot2::ggsave(file=paste0(outputdir, "plots/linegraph_E2_crosscorrelations.png"), line_plot, width = 10, height = 8, dpi = 900) #saves g
```
