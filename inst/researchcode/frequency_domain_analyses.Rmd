...
for (spec in 1:length(deviations)) {
  ...
  title <- paste(paste0("Frequency spectrum plot ", raw_spectra$specifications$experiment[[deviations[[spec]]]]), labels[[spec]], sep = ": ")
  plot(spectrum$specx, spectrum$specy,
...
for (spectrum in 1:length(raw_spectra$HA)) {
  title <- paste0(paste(names(raw_spectra$HA)[[spectrum]], raw_spectra$specifications$experiment[spectrum], sep = " "), " with frequency bins")
...
ggpubr::ggqqplot(df_high, "meanPSD", facet.by = "freqBin", title = "QQ-plot mean power spectral density")
