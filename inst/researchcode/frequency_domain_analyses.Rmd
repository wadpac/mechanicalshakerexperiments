...
createBoxplot <- function(data, outcome = c("domFreq", "meanPSD"), type = c("within", "between"), logscale = c(TRUE, FALSE)){
  ...
  boxplot <- ggpubr::ggboxplot(data, x = "freqBin", y = outcome, color = per,
                               xlab = "Mean frequency bin (Hz)", ylab = label, order = c("t0", "t1", "t2", "t3", "t4",
                                                                                         "t5", "t6", "t7", "t8", "t9",
                                                                                         "t10"), 
...
for (spectrum in 1:length(raw_spectra$HA)) {
  print(names(raw_spectra$HA)[[spectrum]])
...
```{r Model assumptions high sampling frequency experiment}
...
Reference: ActiGraph
```{r Report results for Mean PSD high sampling frequency experiment: ActiGraph}
getOutputMM(meanPSD_intercept_id_high)
```
Reference: MOX
```{r Report results for Mean PSD high sampling frequency experiment: MOX}
...
Reference: GENEActiv
```{r Report results for Mean PSD high sampling frequency experiment: GENEActiv}
df_high$brand <- relevel(df_high$brand, ref = 5) 
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(meanPSD_intercept_id_high)
```
...
```{r Report results for Mean PSD high sampling frequency experiment: Axivity}
df_high$brand <- relevel(df_high$brand, ref = 5) 
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(meanPSD_intercept_id_high)
```
...
```{r Report results for Mean PSD high sampling frequency experiment: activPAL}
df_high$brand <- relevel(df_high$brand, ref = 5) 
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(meanPSD_intercept_id_high)
```
...
```{r Report results for Dominant frequency high sampling frequency experiment: ActiGraph}
df_high$brand <- relevel(df_high$brand, ref = 5) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_high)
...
```{r Report results for Dominant frequency high sampling frequency experiment: MOX}
df_high$brand <- relevel(df_high$brand, ref = 5) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(domFreq_intercept_id_high)
...
```{r Report results for Dominant frequency high sampling frequency experiment: GENEActiv}
df_high$brand <- relevel(df_high$brand, ref = 5) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(domFreq_intercept_id_high)
...
```{r Report results for Dominant frequency high sampling frequency experiment: Axivity}
df_high$brand <- relevel(df_high$brand, ref = 5) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(domFreq_intercept_id_high)
...
```{r Report results for Dominant frequency high sampling frequency experiment: activPAL}
df_high$brand <- relevel(df_high$brand, ref = 5) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, method="ML", data = df_high)
getOutputMM(domFreq_intercept_id_high)
