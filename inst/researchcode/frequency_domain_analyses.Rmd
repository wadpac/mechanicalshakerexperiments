...
meanPSD_interaction_id_bin_low <- lme4::lmer(meanPSD ~ 0 + bin + brand + bin*brand + (1 | serialnumber) + (1 | bin), data = df_long$low)
anova(meanPSD_interaction_id_low, meanPSD_interaction_id_bin_low) # This is not necessary, so use simplest model
```
...
# Evaluate if adding a random intercept for frequency bin is necessary
domFreq_interaction_id__bin_low <- lme4::lmer(domFreq ~ 0 + bin + brand + bin*brand + (1 | serialnumber) + (1 | bin), data = df_long$low)
anova(domFreq_interaction_id_low, domFreq_interaction_id__bin_low) # This is not necessary, so use simplest model
```
...
# Evaluate if adding a random intercept for frequency bin is necessary
meanPSD_interaction_id_bin_high <- lme4::lmer(meanPSD ~ 0 + bin + brand + bin*brand + (1 | serialnumber) + (1 | bin), data = df_long$high)
anova(meanPSD_interaction_id_high, meanPSD_interaction_id_bin_high) # This is not necessary, so use simplest model
```
...
# Evaluate if adding a random intercept for frequency bin is necessary
domFreq_interaction_id_bin_high <- lme4::lmer(domFreq ~ 0 + bin + brand + bin*brand + (1 | serialnumber) + (1 | bin), data = df_long$high)
anova(domFreq_interaction_id_high, domFreq_interaction_id_bin_high) # This is not necessary, so use simplest model
```
##### Model assumption: normal distributions of residuals
