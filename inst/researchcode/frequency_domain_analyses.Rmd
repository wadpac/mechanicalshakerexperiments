...
Here all raw frequency spectra are plotted in one figure.

```{r Frequency spectra low sampling frequency in same plot, echo=FALSE}
p <-  plot(raw.spectra_low[[1]]$specx, raw.spectra_low[[1]]$specy, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l")
for(spectrum in 2:length(raw.spectra_low)){
  p <- p + lines(raw.spectra_low[[spectrum]]$specx, raw.spectra_low[[spectrum]]$specy)
}
```
Here the mean frequency spectrum is plotted.

```{r Mean frequency spectrum low sampling frequency, echo=FALSE}
df_specx <- data.frame()
df_specy <- data.frame()
for(spectrum in 1:length(raw.spectra_low)){
  df_specx <- rbind(df_specx, raw.spectra_low[[spectrum]]$specx)
  df_specy <- rbind(df_specy, raw.spectra_low[[spectrum]]$specy)
}
x_low <- colMeans(df_specx)
y_low <- colMeans(df_specy)

plot(x_low, y_low, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l", main = )
...
```{r Frequency spectra high sampling frequency in same plot, echo=FALSE}
p <-  plot(raw.spectra_high[[1]]$specx, raw.spectra_high[[1]]$specy, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l")
for(spectrum in 2:length(raw.spectra_high)){
  p <- p + lines(raw.spectra_high[[spectrum]]$specx, raw.spectra_high[[spectrum]]$specy)
}
```

Here the mean frequency spectrum is plotted.

```{r Mean frequency spectrum high sampling frequency, echo=FALSE}
df_specx <- data.frame()
df_specy <- data.frame()
for(spectrum in 1:length(raw.spectra_high)){
  df_specx <- rbind(df_specx, raw.spectra_high[[spectrum]]$specx)
  df_specy <- rbind(df_specy, raw.spectra_high[[spectrum]]$specy)
}
x_high <- colMeans(df_specx)
y_high <- colMeans(df_specy)

plot(x_high, y_high, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l", main = )
