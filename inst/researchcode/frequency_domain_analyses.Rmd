...
for (file in 1:length(data)) { # for all accelerometer files
  ...
  rawSpectrum_HA <- deriveSpectrum(HA, sampling_frequency, raw = TRUE, 
                                   file_name = names(data)[[file]])
  raw.spectra_HA[[file]] <- rawSpectrum_HA
}
rm(rawSpectrum_HA)
names(raw.spectra_HA) <- specifications$label  
raw_spectra <- list(HA = raw.spectra_HA, specifications = specifications) # Save derived raw spectra for HA in one list
#cat("\nSaving data...")
...
## Visual inspection
Visual inspection of saved spectrum plots showed clear deviations for the following devices.

```{r Plot deviating spectra, echo=FALSE}
deviations <- c(42, 51, 55, 64, 78, 93)
labels <- raw_spectra$specifications$label[deviations]

for (spec in 1:length(deviations)) {
  spectrum <- raw_spectra$HA[[labels[[spec]]]]
  title <- paste(paste0("Frequency spectrum plot ", 
                        raw_spectra$specifications$experiment[[deviations[[spec]]]]),
                 labels[[spec]], sep = ": ")
  plot(spectrum$specx, spectrum$specy, main = title, 
       xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l") 
}
rm(spec, title, spectrum)
```

### Descriptive information
This table presents the descriptive information of the deviating devices during both experiments.

```{r Descriptive data of deviating spectra, echo=FALSE}
lab <- unique(labels)
...
for (deviation in 1:length(lab)){
  M <- c(M, mean(data[[lab[[deviation]]]]$HA))
  SD <- c(SD, sd(data[[lab[[deviation]]]]$HA))
  min <- c(min, min(data[[lab[[deviation]]]]$HA))
  max <- c(max, max(data[[lab[[deviation]]]]$HA))
}

tab <- cbind(M, SD, min, max)
rownames(tab) <- lab
tab

rm(deviation, lab, M, SD, min, max, tab, labels)
...
Deviating data are removed and excluded from further analyses.
...
raw_spectra$specifications <- specifications
...
save(raw_spectra, file = paste0(datadir, "frequency_spectra_HA_deviates_removed.RData"))
rm(data, specifications, deviations)
```

## Spectrum plots with marked frequency bins

This section presents the spectrum plots of the device acceleration signals included in the analyses with the frequency bins indicated by the red vertical lines.

```{r Plot frequency spectra with frequency bins, echo=FALSE}
#load(paste0(datadir, "frequency_spectra_HA_deviates_removed.RData"))
...
for (spectrum in 1:length(raw_spectra$HA)) {
  title <- paste0(paste(names(raw_spectra$HA)[[spectrum]],
                        raw_spectra$specifications$experiment[spectrum], sep = " "), 
                  " with frequency bins")
  plot(raw_spectra$HA[[spectrum]]$specx, raw_spectra$HA[[spectrum]]$specy,
       main = title, xlab = xlabel, ylab = ylabel, type = "l", xlim = XLIM, bty= "l") 
...
df_wide <- cbind(id, ms_flat_HA_new$specifications$brand, ms_flat_HA_new$specifications$experiment, ms_flat_HA_new$specifications$sampling_frequency,
                 ms_flat_HA_new$specifications$dynamic_range)
...
This section presents the statistical comparison of the spectrum plots in terms of the aggregated dominant frequency and mean power spectral density for each frequency bin per sampling frequency experiment.
...
bxp_domfreq_low <- createBoxplot(df_low, outcome = "domFreq", type = "between", logscale = FALSE)
bxp_psd_low <- createBoxplot(df_low, outcome = "meanPSD", type = "between", logscale = FALSE)
...
bxp_domfreq_low_log <- createBoxplot(df_low, outcome = "domFreq", type = "between", 
                                     logscale = TRUE)
bxp_psd_low_log <- createBoxplot(df_low, outcome = "meanPSD", type = "between", logscale = TRUE) 
...
### Boxplots
In this section boxplots for both outcomes (dominant frequency and mean power spectral density) are presented.
...
In this section the statistical comparison between the brand for the low sampling frequency experiment is presented. Thee mixed models for mean power spectral density and dominant frequency with a random intercept for serial number (id) are presented separately.
...
ggpubr::ggqqplot(df_low, "meanPSD", facet.by = "freqBin", title = "QQ-plot mean power spectral density")
...
ggpubr::ggqqplot(df_low, "domFreq", facet.by = "freqBin", title = "QQ-plot dominant frequency")
...
bxp_domfreq_high <- createBoxplot(df_high, outcome = "domFreq", type = "between", 
                                  logscale = FALSE)
bxp_psd_high <- createBoxplot(df_high, outcome = "meanPSD", type = "between", logscale = FALSE) 
...
bxp_domfreq_high_log <- createBoxplot(df_high, outcome = "domFreq", type = "between", 
                                      logscale = TRUE)
bxp_psd_high_log <- createBoxplot(df_high, outcome = "meanPSD", type = "between", logscale = TRUE)
...
### Mixed model analyses
In this section the statistical comparison between the brand for the high sampling frequency experiment is presented. Thee mixed models for mean power spectral density and dominant frequency with a random intercept for serial number (id) are presented separately.
...
ggpubr::ggqqplot(df_high, "domFreq", facet.by = "freqBin", title = "QQ-plot dominant frequency")
