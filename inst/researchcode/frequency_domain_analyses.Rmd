...
createBoxplot <- function(data, freqBins, outcome = c("domFreq", "meanPSD"), experiment = c("low", "high"), relative = c(TRUE, FALSE)){
  ...
  if(length(unique(data$brand)) == 5) {
    palette = c("red", "orange", "green", "blue", "purple")
  }
  else if(length(unique(data$brand)) == 3) {
    palette = c("red", "green", "blue")
  }
  
  boxplot <- ggpubr::ggboxplot(data, x = "bin", y = outcome, color = "brand", palette = palette,
                               xlab = "Shaker frequency (Hertz)", ylab = label, 
                                order = bins,
                               font.label = list(size = 10, color = "black")) +
    ggpubr::font("xlab", size = 10, color = "black") +
    ggpubr::font("x.text", size = 10, color = "black") + 
...
bxp_psd_high <- bxp_psd_high + ggplot2::labs(caption = "b) High sampling frequency experiment") + ggplot2::theme(legend.position="none", plot.caption=element_text(hjust = 0.5, size = 10)) 
bxp_psd_low <- bxp_psd_low + ggplot2::labs(caption = "a) Low sampling frequency experiment") + ggplot2::theme(legend.position="bottom", plot.caption=element_text(hjust = 0.5, size = 10)) 
legend <- ggpubr::get_legend(bxp_psd_low)
  bxp_psd_low <- bxp_psd_low + ggplot2::theme(legend.position="none")

g <- gridExtra::arrangeGrob(bxp_psd_low, bxp_psd_high,legend,
                            nrow = 3,
                            heights = c(3, 3, 0.2)) #generates g

#ggplot2::ggsave(plot = g, filename = "boxplot_meanPSD")
ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_meanPSD.png"), g, width = 10, height = 8, dpi = 900) #saves g
```


```{r Create boxplot dominant frequency, include=FALSE}
bxp_domfreq_high <- bxp_domfreq_high + ggplot2::labs(caption = "b) High sampling frequency experiment") + ggplot2::theme(legend.position="none", plot.caption=element_text(hjust = 0.5, size = 10)) 
bxp_domfreq_low <- bxp_domfreq_low + ggplot2::labs(caption = "a) Low sampling frequency experiment") + ggplot2::theme(legend.position="bottom", plot.caption=element_text(hjust = 0.5, size = 10)) 
legend <- ggpubr::get_legend(bxp_domfreq_low)
  bxp_domfreq_low <- bxp_domfreq_low + ggplot2::theme(legend.position="none")

g <- gridExtra::arrangeGrob(bxp_domfreq_low, bxp_domfreq_high, legend,
                            nrow = 3,
                            heights = c(2.5, 2.5, 0.2)) #generates g

ggplot2::ggsave(file=paste0(datadir, "plots/boxplot_domFreq.png"), g, width = 10, height = 8, dpi = 900)
```
