...
#' @param percentage Boolean to indicate whether the values need to be plotted with respect to the frequency bin mean. One of c("TRUE", "FALSE") 
#' @return Boxplot between or within devices for the outcome (y-axis) per frequency bin (x-axis)
#' @export
createBoxplot <- function(data, freqBins, outcome = c("domFreq", "meanPSD"), experiment = c("low", "high"), percentage = c(TRUE, FALSE)){
  if (outcome == "domFreq" & percentage == FALSE){
    label = "Dominant frequency (Hertz)"
  } else if (outcome == "meanPSD" & percentage == FALSE){
    label = "Mean power spectral density (g2/Hertz)"
  } else if (outcome == "domFreq" & percentage == TRUE){
  ...
  if(percentage == TRUE){
    if(outcome == "domFreq"){
      data <- transform(data, perc = ave(domFreq, bin.labels, FUN = prop.table)*100)
    } else{
...
bxp_domfreq_low <- createBoxplot(df_long$low, bins_low, outcome = "domFreq", experiment = "low", percentage = FALSE)
bxp_psd_low <- createBoxplot(df_long$low, bins_low, outcome = "meanPSD", experiment = "low", percentage = FALSE)
...
bxp_domfreq_low_perc <- createBoxplot(df_long$low, bins_low, outcome = "domFreq", experiment = "low", percentage = TRUE)
bxp_psd_low_perc <- createBoxplot(df_long$low, bins_low, outcome = "meanPSD", experiment = "low", percentage = TRUE)
...
bxp_domfreq_high <- createBoxplot(df_long$high, bins_high, outcome = "domFreq", experiment = "high", percentage = FALSE)
bxp_psd_high <- createBoxplot(df_long$high, bins_high, outcome = "meanPSD", experiment = "high", percentage = FALSE) 
...
bxp_domfreq_high_perc <- createBoxplot(df_long$high, bins_high, outcome = "domFreq", experiment = "high", percentage = TRUE)
bxp_psd_high_perc <- createBoxplot(df_long$high, bins_high, outcome = "meanPSD", experiment = "high", percentage = TRUE)
