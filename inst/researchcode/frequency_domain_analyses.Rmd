...
#' deriveSpectrum
#'
#' @description 'deriveSectrum' derives the frequency spectrum of a signal (called from within derivePeakIntervals to derive smoothed spectrum)
#'
...
In this section the raw frequency spectra are printed for each accelerometer recording  and for both experiments separately.
...
for (file in 1:length(clean_data_low$data)) { # for all accelerometer files
  ...
  rawSpectrum <- deriveSpectrum(HA, sampling_frequency, raw = TRUE, 
                                   file_name = names(clean_data_low$data)[[file]])
...
# Note: activPAL was set to a sampling frequency of 20 Hz (in both experiments), however, these data were considered for the high sampling frequency experiment as the recorded amplitude was expected to be the same.
...
for (file in 1:length(clean_data_high$data)) { # for all accelerometer files
  ...
  rawSpectrum <- deriveSpectrum(HA, sampling_frequency, raw = TRUE, 
                                   file_name = names(clean_data_high$data)[[file]])
...
raw_spectra <- list(high = raw.spectra_high, specifications_high = clean_data_high$specifications,
                    low = raw.spectra_low, specifications_low = clean_data_low$specifications) # Save derived raw spectra for HA in one list
#cat("\nSaving data...")
...
## Frequency bins
The marked rotations per minute (rpm) on the analogical shaker machines' turning wheel were divided by 60 to derive the associated frequency in Hertz. The marked rpm were: 30, 50, 75, 100, 125, 150, 175, 200, 225, and 250. This resulted in the following associated frequencies: 0.50, 0.83, 1.25, 1.67, 2.08, 2.50, 2.92, 3.33, 3.75, and 4.17. 
...
df_long$high$brand <- relevel(df_long$high$brand, ref = 4) 
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long$high)
getOutputMM(meanPSD_intercept_id_high)
```
...
## High sampling frequency experiment (activPAL excluded)

### Boxplots
This section presents the boxplots for both outcomes (dominant frequency and mean power spectral density) for the high sampling frequency experiment.

```{r Exclude activPAL data, include=FALSE}
df_long_high_excl_aP <- df_long$high[!df_long$high$brand =="Activpal",]
```


```{r Create boxplots high sampling frequency ap, include=FALSE}
bxp_domfreq_high_ap <- createBoxplot(df_long_high_excl_aP, outcome = "domFreq", type = "between", 
                                  logscale = FALSE)
bxp_psd_high_ap <- createBoxplot(df_long_high_excl_aP, outcome = "meanPSD", type = "between", logscale = FALSE) 

gridExtra::grid.arrange(bxp_domfreq_high_ap, bxp_psd_high_ap, nrow=1) #arranges plots within grid
g <- gridExtra::arrangeGrob(bxp_domfreq_high_ap + ggplot2::theme(legend.position="none"), 
                            bxp_psd_high_ap + ggplot2::theme(legend.position="bottom"), nrow=2) #generates g
ggplot2::ggsave(file=paste0(datadir, "plots/boxplots_HA_HIGH_ap.pdf"), g) #saves g

bxp_domfreq_high_ap_log <- createBoxplot(df_long_high_excl_aP, outcome = "domFreq", type = "between", 
                                      logscale = TRUE)
bxp_psd_high_ap_log <- createBoxplot(df_long_high_excl_aP, outcome = "meanPSD", type = "between", logscale = TRUE)
gridExtra::grid.arrange(bxp_psd_high_ap_log, bxp_psd_high_log, 
                        nrow=1) #arranges plots within grid
g_log <- gridExtra::arrangeGrob(bxp_domfreq_high_ap_log + ggplot2::theme(legend.position="none"),
                                bxp_psd_high_ap_log + ggplot2::theme(legend.position="bottom"), 
                                nrow=2) #generates g_log
ggplot2::ggsave(file=paste0(datadir, "plots/boxplots_HA_HIGH_ap_logscale.pdf"), g_log) #saves g_log
```

Boxplot

```{r Print boxplot high sampling frequency experiment ap, echo=FALSE}
plot(g)
```

Boxplot with log scale

```{r Print log boxplot high sampling frequency experiment ap, echo=FALSE}
plot(g_log)
```
...
```{r Mixed Model Analyses high sampling frequency experiment ap, include=FALSE}
# Starting model: with random intercept for accelerometer id (to adjust for the repeated measurements of the accelerometer devices)
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long_high_excl_aP)
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long_high_excl_aP)
...
#### Model assumption: normal distributions of residuals
```{r Mean PSD Model assumptions high sampling frequency experiment ap, echo=FALSE}
ggpubr::ggqqplot(df_long_high_excl_aP, "meanPSD", facet.by = "freqBin", 
                 title = "QQ-plot mean power spectral density")
residuals_meanPSD <- resid(meanPSD_intercept_id_high)
#summary(residuals_meanPSD)
hist(residuals_meanPSD)
...
```{r Dominant frequency Model assumptions high sampling frequency experiment ap, echo=FALSE}
ggpubr::ggqqplot(df_long_high_excl_aP, "domFreq", facet.by = "freqBin", title = "QQ-plot dominant frequency")
residuals_domFreq <- resid(domFreq_intercept_id_high)
#summary(residuals_domFreq)
hist(residuals_domFreq) # approximately normally distributed
```
...
##### Reference: Axivity
```{r Report results for Mean PSD high sampling frequency experiment ap: Axivity, echo=FALSE}
getOutputMM(meanPSD_intercept_id_high)
```
##### Reference: GENEActiv
```{r Report results for Mean PSD high sampling frequency experiment ap: GENEActiv, echo=FALSE}
df_long_high_excl_aP$brand <- relevel(df_long_high_excl_aP$brand, ref = 3) 
meanPSD_intercept_id_high <- nlme::lme(meanPSD ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long_high_excl_aP)
getOutputMM(meanPSD_intercept_id_high)
```
...
```{r Report results for Dominant frequency high sampling frequency experiment ap: GENEActiv, echo=FALSE}
df_long_high_excl_aP$brand <- relevel(df_long_high_excl_aP$brand, ref = 3) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long_high_excl_aP)
getOutputMM(domFreq_intercept_id_high)
...
```{r Report results for Dominant frequency high sampling frequency experiment ap: Axivity, echo=FALSE}
df_long_high_excl_aP$brand <- relevel(df_long_high_excl_aP$brand, ref = 3) 
domFreq_intercept_id_high <- nlme::lme(domFreq ~ brand, random = ~ 1|id, 
                                       method="ML", data = df_long_high_excl_aP)
getOutputMM(domFreq_intercept_id_high)
