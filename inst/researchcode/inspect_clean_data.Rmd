...
#' @export
#' 
plotSignals <- function(directory, data, includeShakerConditionNULL = TRUE){
  plot_list <- list()
  for(signal in 1:length(data)) {
    if (includeShakerConditionNULL == TRUE){
          file_name <- paste0("time series (include baseline): ", names(data)[signal])
    }
    else {
...
#' @export
#' 
cleanSignals <- function(data, includeShakerConditionNULL = TRUE) {
  cleaned_signals <- list()
  
  for(file in 1:length(data$data)){
    signal <- data$data[[file]]
    #Step 1: Remove data on which the experiment was stopped (shaking frequency -1)
    cleaned_signal <- signal[which(signal$shaking_frequency != "-1"),]

     #Step 2: If the data will be used for the time domain, do not include the noise around 0 Hz as this will be correlated with noise around 0 Hz (includeShakerConditionNULL == FALSE). If it is used for the frequency domain, include this shaker condition (includeShakerConditionNULL == TRUE)
    if(includeShakerConditionNULL == FALSE){
      cleaned_signal <- cleaned_signal[which(cleaned_signal$shaking_frequency != "0"),]
    }
    
    #Step 3: If the experiment is the high sampling frequency experiment, remove the double data due to repeating the experiment.
...
# Cleaning the data
Remove the data for which the shaking frequency was null in order to avoid comparison of the signal noise during time domain analyses. This null shaker condition will be included for the frequency domain analyses. In addition we removed repeated measurements for some of the shaking frequencies because a device got loose during the high sampling frequency experiment.

```{r Clean the signals, include = FALSE}
data_cleaned_timeDomain <- cleanSignals(ms_flat_HA_aP_excluded, includeShakerConditionNULL = FALSE)
data_cleaned_frequencyDomain <- cleanSignals(ms_flat_HA_aP_excluded, includeShakerConditionNULL = TRUE)
...
# Save a plot for each signal
plots_clean_timeDomain <- plotSignals(datadir, data_cleaned_timeDomain$data, includeShakerConditionNULL = FALSE)
plots_clean_frequencyDomain <- plotSignals(datadir, data_cleaned_frequencyDomain$data, includeShakerConditionNULL = TRUE)
```

### Time domain signals
```{r Plot all time domain signals, echo = FALSE}
# Save a plot for each signal
for(plot in 1:length(plots_clean_timeDomain)){
  print(plots_clean_timeDomain[[plot]])
}
rm(plot, plots_clean_timeDomain)
```

### Frequency domain signals
```{r Plot all frequency domain signals, echo = FALSE}
# Save a plot for each signal
for(plot in 1:length(plots_clean_frequencyDomain)){
  print(plots_clean_frequencyDomain[[plot]])
}
rm(plot, plots_clean_frequencyDomain)
```
...
data_low_timeDomain <- data_cleaned_timeDomain$data[data_cleaned_timeDomain$specifications$experiment == "ms_lfcr"]
specifications_low_timeDomain <- data_cleaned_timeDomain$specifications[data_cleaned_timeDomain$specifications$experiment == "ms_lfcr", ]
check_low_timeDomain <- checkStartEnd(data_low_timeDomain, threshold = 1, unit = "sec") #there are no devices for which the start or end times differ with more than 1 sec

data_low_frequencyDomain <- data_cleaned_frequencyDomain$data[data_cleaned_frequencyDomain$specifications$experiment == "ms_lfcr"]
specifications_low_frequencyDomain <- data_cleaned_frequencyDomain$specifications[data_cleaned_frequencyDomain$specifications$experiment == "ms_lfcr", ]
check_low_frequencyDomain <- checkStartEnd(data_low_frequencyDomain, threshold = 1, unit = "sec") #there are no devices for which the start or end times differ with more than 1 sec

clean_data_low <- list(timeDomain = list(data = data_low_timeDomain, specifications = specifications_low_timeDomain), frequencyDomain = list(data = data_low_frequencyDomain, specifications = specifications_low_frequencyDomain))

rm(data_low_timeDomain, specifications_low_timeDomain, data_low_frequencyDomain, specifications_low_frequencyDomain)
```
#### Absolute start time differences (in seconds)
Time domain
```{r Low sampling experiment start times time domain, echo=FALSE}
print(as.data.frame(check_low_timeDomain$diff_start))
```
Frequency domain
```{r Low sampling experiment start times frequency domain, echo=FALSE}
print(as.data.frame(check_low_frequencyDomain$diff_start))
```

#### Absolute end time differences (in seconds)
Time domain
```{r Low sampling experiment end times time domain, echo=FALSE}
print(as.data.frame(check_low_timeDomain$diff_end))
rm(check_low_timeDomain)
```
Frequency domain
```{r Low sampling experiment end times frequency domain, echo=FALSE}
print(as.data.frame(check_low_frequencyDomain$diff_end))
rm(check_low_frequencyDomain)
```
...
data_high_timeDomain <- data_cleaned_timeDomain$data[data_cleaned_timeDomain$specifications$experiment == "ms_hfcr"]
specifications_high_timeDomain <- data_cleaned_timeDomain$specifications[data_cleaned_timeDomain$specifications$experiment == "ms_hfcr", ]
check_high_timeDomain <- checkStartEnd(data_high_timeDomain, threshold = 1, unit = "sec") 
print(as.data.frame(check_high_timeDomain$diff_end))

data_high_frequencyDomain <- data_cleaned_frequencyDomain$data[data_cleaned_frequencyDomain$specifications$experiment == "ms_hfcr"]
specifications_high_frequencyDomain <- data_cleaned_frequencyDomain$specifications[data_cleaned_frequencyDomain$specifications$experiment == "ms_hfcr", ]
check_high_frequencyDomain <- checkStartEnd(data_high_frequencyDomain, threshold = 1, unit = "sec") 
print(as.data.frame(check_high_frequencyDomain$diff_end))

# For MOX devices, the time difference was 31.99984 mins
# All MOX devices quit early, so leave them out the high sampling frequency experiment comparisons
data_high_timeDomain[check_high_timeDomain$names_end] <- NULL
specifications_high_timeDomain <- specifications_high_timeDomain[!specifications_high_timeDomain$label %in% check_high_timeDomain$names_end ,]
data_high_frequencyDomain[check_high_frequencyDomain$names_end] <- NULL
specifications_high_frequencyDomain <- specifications_high_frequencyDomain[!specifications_high_frequencyDomain$label %in% check_high_frequencyDomain$names_end ,]


clean_data_high <- list(timeDomain = list(data = data_high_timeDomain, specifications = specifications_high_timeDomain), frequencyDomain = list(data = data_high_frequencyDomain, specifications = specifications_high_frequencyDomain))


rm(data_high_timeDomain, data_high_frequencyDomain, specifications_high_timeDomain, specifications_high_frequencyDomain, data_cleaned_frequencyDomain, data_cleaned_timeDomain)
```

#### Absolute start time differences (in seconds)
Time domain
```{r High sampling experiment time domain start times, echo=FALSE}
print(as.data.frame(check_high_timeDomain$diff_start))
```
Frequency domain
```{r High sampling experiment frequency domain start times, echo=FALSE}
print(as.data.frame(check_high_frequencyDomain$diff_start))
```
#### Absolute end time differences (in seconds)
Time domain
```{r High sampling experiment time domain end times, echo=FALSE}
print(as.data.frame(check_high_timeDomain$diff_end))
rm(check_high_timeDomain)
```
Frequency domain
```{r High sampling experiment frequency domain end times, echo=FALSE}
print(as.data.frame(check_high_frequencyDomain$diff_end))
rm(check_high_frequencyDomain)
```
