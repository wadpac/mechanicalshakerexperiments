...
plot_list <- plotSignals(datadir, ms_flat_HA$data)
rm(filename_flatHA)
```
...
deviates <- which(names(ms_flat_HA$data) %in% c("aP_490", "aP_254", "Ax_406"))
M <- c()
SD <- c()
min <- c()
max <- c()
...
for (deviation in 1:length(deviates)){
  M <- c(M, mean(ms_flat_HA$data[[deviates[[deviation]]]]$HA))
  SD <- c(SD, sd(ms_flat_HA$data[[deviates[[deviation]]]]$HA))
  min <- c(min, min(ms_flat_HA$data[[deviates[[deviation]]]]$HA))
  max <- c(max, max(ms_flat_HA$data[[deviates[[deviation]]]]$HA))
  experiment <- c(experiment, ms_flat_HA$specifications$experiment[deviates[deviation]])
}
tab <- cbind(experiment, M, SD, min, max)
rownames(tab) <- names(ms_flat_HA$data[deviates])
tab
...
ms_flat_HA$data[deviates] <- NULL
ms_flat_HA$specifications <- ms_flat_HA$specifications[-deviates,]

cat("\nSaving data...")
save(ms_flat_HA, file = paste0(datadir, "ms_flat_HA_deviates_removed.RData"))
rm(deviates)
```
...
data_cleaned <- cleanSignals(ms_flat_HA)
rm(ms_flat_HA)
```
