...
computeCrossCorrelations <- function(data, plot) {
  ...
  for (x in 1:length(data)) {
    for (y in 1:length(data)) {
      ...
      #If one the time series during the shaker condition(s) has a standard deviation of 0, cross-correlation coefficient = 0
...
```{r Heatmap low shaking frequency experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=14}
fullmatrices_low <- correlation_matrices_perShakerCondition$low

for (condition in 1:length(fullmatrices_low)){
  corrplot::corrplot(pmax(fullmatrices_low[[condition]]$correlationMatrix,
...
for (condition in 1:length(fullmatrices_high)){
  corrplot::corrplot(pmax(fullmatrices_high[[condition]]$correlationMatrix,
