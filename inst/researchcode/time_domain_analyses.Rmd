...
# Load mechanical shaker data for horizontal axis without the deviating signals
...
specifications_high <- ms_flat_HA_new$specifications[ms_flat_HA_new$specifications$experiment == "ms_hfcr" &                      
                        round(as.numeric(ms_flat_HA_new$specifications$sampling_frequency)) == "100",]
specifications_low <- ms_flat_HA_new$specifications[ms_flat_HA_new$specifications$experiment == "ms_lfcr" &                                                 round(as.numeric(ms_flat_HA_new$specifications$sampling_frequency)) == "25",]
    ...
    for (y in 1:ncol(correlationMatrix)){
      if (correlationMatrix[x,y] < 1 & !is.na(correlationMatrix[x,y])) {
        if(correlationMatrix[x,y] <= threshold){ #only create pairwise plots if the cross-correlation is equal/lower to the threshold
          ...
          file_name <- paste(paste(names(data)[[x]], names(data)[[y]], sep = "_vs_"), correlationMatrix[x,y], sep = " r = ")
        jpeg(paste(datadir, file_name, ".jpeg", sep = "/plots/"), width=600, height=500, res=120) 
        p <- ggplot2::ggplot() +
          ggplot2::geom_point(data = data[[x]], ggplot2::aes(x = time, y = HA), color = "black", alpha = 0.4, size = 0.25) +
          ggplot2::geom_point(data = data[[y]], ggplot2::aes(x = time, y = HA), color = "red", alpha =  0.4, size = 0.25, show.legend = TRUE) +
          ggplot2::scale_fill_manual(values = c(device1 = "black", device2 = "red"), labels = c(device1, device2)) +
          ggplot2::ggtitle(file_name) + ggplot2::theme_light() 
        dev.off()
        plot_list[[counter]] <- p
        }
      }
...
rm(correlations_low, correlations_high)

#load(paste0(datadir, "cross_correlations.RData"))
```

# Cross-correlations
In this section the maximum lagged cross-correlations between all signal pairs are presented for both experiments separately.
...
heatmap_low <- corrplot::corrplot(pmax(fullmatrix_low, t(fullmatrix_low), na.rm = TRUE), 
                                  ...
                                  number.cex = 2, cl.cex = 2, tl.cex = 2,
...
In this section pairwise plots are generated for cross-correlations that are 1 SD lower than the mean cross-correlation.
...
min <- min(correlation_matrices$low$correlationMatrix, na.rm = TRUE)
max <- max(correlation_matrices$low$correlationMatrix, na.rm = TRUE)
crosscorr_low_threshold <- M - SD
cross_correlations <- c(M, SD, min, max, crosscorr_low_threshold)

# lags
M <- mean(correlation_matrices$low$lags, na.rm = TRUE)
SD <- sd(correlation_matrices$low$lags, na.rm = TRUE)
min <- min(correlation_matrices$low$lags, na.rm = TRUE)
max <- max(correlation_matrices$low$lags, na.rm = TRUE)
low_threshold <- NA
lags <- c(M, SD, min, max, low_threshold)

tab <- rbind(cross_correlations, lags)
colnames(tab) <- c("M", "SD", "min", "max", "threshold")
tab
rm(tab, M, SD, min, max, low_threshold)
```
...
plots_low <- pairwisePlots(correlationMatrix = correlation_matrices$low$correlationMatrix, 
                           data = data_low_25, threshold = crosscorr_low_threshold)
```
...
min <- min(correlation_matrices$high$correlationMatrix, na.rm = TRUE)
max <- max(correlation_matrices$high$correlationMatrix, na.rm = TRUE)
crosscorr_high_threshold <- M - SD
cross_correlations <- c(M, SD, min, max, crosscorr_high_threshold)

# lags
M <- mean(correlation_matrices$high$lags, na.rm = TRUE)
SD <- sd(correlation_matrices$high$lags, na.rm = TRUE)
min <- min(correlation_matrices$high$lags, na.rm = TRUE)
max <- max(correlation_matrices$high$lags, na.rm = TRUE)
threshold_high <- NA
lags <- c(M, SD, min, max, threshold_high)

tab <- rbind(cross_correlations, lags)
colnames(tab) <- c("M", "SD", "min", "max", "threshold")
tab
rm(tab, M, SD, min, max, threshold_high)
```
...
plots_high <- pairwisePlots(correlationMatrix = correlation_matrices$high$correlationMatrix, 
                            data = data_high_100, threshold = crosscorr_high_threshold)
```
...
### Data crop
The pairwise plots indicate that the maximum lagged cross-corrlationas are low due to repetition of some measurements in the high sampling frequency experiment because one GENEActiv device got loose. Therefore, we repeated the analyses for cropped signals (i.e., data of the time points for which all devices were included during the shaking frequencies).

```{r Crop signals, include=FALSE}
data_high_100_crop <- list()

tz <- "Europe/Amsterdam"
#Times from experiment file where accelerometers_used = all
...
for(file in 1:length(data_high_100)){
  data_high_crop1 <- data_high_100[[file]][which(data_high_100[[file]]$time >= start1 & data_high_100[[file]]$time <= end1),]
  data_high_crop2 <- data_high_100[[file]][which(data_high_100[[file]]$time >= start2 & data_high_100[[file]]$time <= end2),]
  data_high_100_crop[[file]] <- rbind(data_high_crop1, data_high_crop2)
}

names(data_high_100_crop) <- names(data_high_100)

save(data_high_100_crop, file = paste0(datadir, "data_high_100_cropped.RData"))
```

```{r Compute cross-correlation matrices for cropped signal, include = FALSE}
correlations_high_crop <- computeCrossCorrelations(data_high_100_crop, plot = TRUE)
save(correlations_high_crop, file = paste0(datadir, "cross_correlations_high_crop.RData"))
```

#### Cross-correlations
In this section the maximum lagged cross-correlations between all cropped signal pairs are presented for the high sampling experiment.

##### Heatmap
```{r Heatmap high sampling frequency experiment cropped, echo=FALSE, fig.width=14, fig.height=14}
fullmatrix_high_crop <- correlations_high_crop$correlationMatrix
heatmap_high_crop <- corrplot::corrplot(pmax(fullmatrix_high_crop, t(fullmatrix_high_crop), na.rm = TRUE), 
                                  type = 'lower', method = 'color', order = "alphabet", 
                                  number.cex = 1.25, cl.cex = 2, tl.cex = 2,
                                  col.lim = c(0,1), addCoef.col = 'black', tl.srt = 45, 
                                  diag = TRUE, tl.col = "black")
rm(fullmatrix_high_crop, heatmap_high_crop)
```
##### Pairwise signal plots
In this section pairwise plots are generated for cross-correlations that are 1 SD lower than the mean cross-correlation.
...
```{r Summary statistics high sampling frequency experiment cropped signals, echo=FALSE}
# cross-correlations
M <- mean(correlations_high_crop$correlationMatrix, na.rm = TRUE)
SD <- sd(correlations_high_crop$correlationMatrix, na.rm = TRUE)
min <- min(correlations_high_crop$correlationMatrix, na.rm = TRUE)
max <- max(correlations_high_crop$correlationMatrix, na.rm = TRUE)
crosscorr_high_crop_threshold <- M - SD
cross_correlations <- c(M, SD, min, max, crosscorr_high_crop_threshold)

# lags
M <- mean(correlations_high_crop$lags, na.rm = TRUE)
SD <- sd(correlations_high_crop$lags, na.rm = TRUE)
min <- min(correlations_high_crop$lags, na.rm = TRUE)
max <- max(correlations_high_crop$lags, na.rm = TRUE)
threshold_high <- NA
lags <- c(M, SD, min, max, threshold_high)
...
```{r High Sampling Frequency Experiment: Pairwise plots cropped signals, include=FALSE}
plots_high_crop <- pairwisePlots(correlationMatrix = correlations_high_crop$correlationMatrix, 
                            data = data_high_100_crop, threshold = crosscorr_high_crop_threshold)
```

###### Pairwise plots

```{r Print pairwise plots: high sampling experiment cropped signals, echo=FALSE}
for(plot in 1:length(plots_high_crop)){
  print(plots_high_crop[[plot]])
}
```
...
df_correlations_high_crop <- dataframeShapeComparisonBeween(correlations_high_crop, specifications_high) 
...
save(df_correlations_high_crop, file = paste0(datadir, "cross_correlation_df_high_crop.RData"))
...
correlations_between_high_cropped <- df_correlations_high_crop[df_correlations_high_crop$same_brand =="0", ]
```
...
### High sampling frequency experiment
```{r Descriptives high sampling frequency experiment, echo=FALSE}
library(dplyr)
...
tab <- rbind(avg_cor$AxivityActigraph, avg_cor$GENEActivActigraph, avg_cor$GENEActivAxivity, avg_cor$MOXActigraph, avg_cor$MOXAxivity, avg_cor$MOXGENEActiv)
rownames(tab) <- c("AxivityActigraph", "GENEActivActigraph", "GENEActivAxivity", "MOXActigraph", "MOXAxivity", "MOXGENEActiv")
tab <- tab[,-2]
tab <- tab[,-4]
...
### High sampling frequency experiment cropped signals
```{r Descriptives high sampling frequency experiment cropped, echo=FALSE}
library(dplyr)
correlations_between_high_cropped$correlation <- as.numeric(correlations_between_high_cropped$correlation)
# table_between_high_crop <- correlations_between_high_cropped %>% group_by(brands)
avg_cor <- tapply(correlations_between_high_cropped$correlation, correlations_between_high_cropped$brands, summary)
SD <- tapply(correlations_between_high_cropped$correlation, correlations_between_high_cropped$brands, sd)
