  ...
  for (x in 1:length(data)) {
    for (y in 1:length(data)) {
      ...
      #If one of the time series during the shaker condition(s) has a standard deviation of 0, cross-correlation coefficient = 0
        ...
        else if(sd(data[[x]]$HA) == 0 & sd(data[[y]]$HA) == 0){
          correlationMatrix[x,y] <- 1
        }
        else{
...
For the calculation of the cross-correlation coefficients, only data of devices with equal sampling frequency were included. For the high sampling frequency experiment this sampling frequency was 100 Hz, and for the low frequency sampling experiment this was 25 Hz. This section presents the maximum lagged cross-correlation coefficients between all signal pairs separately for both experiments.
...
correlation_matrices_perShakerCondition <- list(low = correlations_low_perShakerCondition, high = correlations_high_perShakerCondition)
save(correlation_matrices_perShakerCondition, file = paste0(datadir, "cross_correlations_perShakerCondition.RData"))
...
#load(paste0(datadir, "cross_correlations_perShakerCondition.RData"))
```

## Low sampling frequency experiment

### Overall heatmap
```{r Heatmap low sampling frequency experiment, echo=FALSE, fig.width=14, fig.height=14}
...
```{r Heatmap low sampling frequency experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=15}
fullmatrices_low <- correlation_matrices_perShakerCondition$low

heatmaps_low <- list()
for (condition in 1:length(fullmatrices_low)){
  heatmaps_low[[condition]] <- corrplot::corrplot(pmax(fullmatrices_low[[condition]]$correlationMatrix,
                          t(fullmatrices_low[[condition]]$correlationMatrix), na.rm = TRUE), 
                            title = paste0("shaker condition: ", names(fullmatrices_low)[condition]),
                            type = 'lower', method = 'color', order = "alphabet", 
                            number.cex = 1.25, cl.cex = 2, tl.cex = 2,
...
rm(fullmatrices_low, heatmaps_low)
```
### Comparison
This section presents the average maximum lagged cross-correlation between and within brands separately per shaker condition.

```{r Calculate average maximum cross lagged correlation coefficients low sampling frequency experiment per shaker condition, include=FALSE}
avg_crosscor_low_between <- list()
avg_crosscor_low_within <- list()

for (condition in 1:length(correlation_matrices_perShakerCondition$low)){
  avg_crosscor_low <- dataframeShapeComparisonBeween(correlation_matrices_perShakerCondition$low[[condition]], specifications_low) 
  avg_crosscor_low_between[[condition]] <- avg_crosscor_low[avg_crosscor_low$same_brand == "0",]
  avg_crosscor_low_within[[condition]] <- avg_crosscor_low[avg_crosscor_low$same_brand == "1",]
}
rm(avg_crosscor_low)
names(avg_crosscor_low_between) <- names(correlation_matrices_perShakerCondition$low)
names(avg_crosscor_low_within) <- names(correlation_matrices_perShakerCondition$low)
```

#### Between brands
```{r Between brand comparison low sampling frequency experiment, echo=FALSE}
table_between_low <- list()
for(condition in 1:length(avg_crosscor_low_between)){
  df_correlations_low <- avg_crosscor_low_between[[condition]]
  df_correlations_low$correlation <- as.numeric(df_correlations_low$correlation)
  avg_cor <- tapply(df_correlations_low$correlation, df_correlations_low$brands, summary)
  
  tab <- rbind(avg_cor$GENEActivAxivity, avg_cor$MOXAxivity, avg_cor$MOXGENEActiv)
  tab <- tab[,-1]
  tab <- tab[,-5]
  SD <- tapply(df_correlations_low$correlation, df_correlations_low$brands, sd)
  SD <- c(SD[2], SD[4], SD[5])
  tab <- cbind(tab, SD)
  table_between_low[[condition]] <- tab
}
rm(avg_cor, tab, SD)
names(table_between_low) <- names(correlation_matrices_perShakerCondition$low)
```

```{r Print between comparison tables low frequency experiment, echo = FALSE}
for(condition in 1:length(table_between_low)){
  print(paste0("Shaker condition: ", names(table_between_low)[condition]))
  print(table_between_low[[condition]])
}
```

#### Within brands
```{r Within brand comparison low sampling frequency experiment, echo=FALSE}
table_within_low <- list()
for(condition in 1:length(avg_crosscor_low_within)){
  df_correlations_low <- avg_crosscor_low_within[[condition]]
  df_correlations_low$correlation <- as.numeric(df_correlations_low$correlation)
  avg_cor <- tapply(df_correlations_low$correlation, df_correlations_low$brands, summary)
  
  tab <- rbind(avg_cor$AxivityAxivity, avg_cor$GENEActivGENEActiv, avg_cor$MOXMOX)
  tab <- tab[,-1]
  tab <- tab[,-5]
  SD <- tapply(df_correlations_low$correlation, df_correlations_low$brands, sd)
  SD <- c(SD[1], SD[3], SD[6])
  tab <- cbind(tab, SD)
  table_within_low[[condition]] <- tab
}
rm(avg_cor, tab, SD)
names(table_within_low) <- names(correlation_matrices_perShakerCondition$low)
```

```{r Print within comparison tables low frequency experiment, echo = FALSE}
for(condition in 1:length(table_within_low)){
  print(paste0("Shaker condition: ", names(table_within_low)[condition]))
  print(table_within_low[[condition]])
}
```
...
#### Heatmap table average cross-correlation coefficients per shaker condition
```{r Heatmap table low sampling freqency, echo=TRUE}
overview_tab_between <- data.frame()
overview_tab_within <- data.frame()

for(condition in 1:length(table_between_low)){
  overview_tab_between <- rbind(overview_tab_between, table_between_low[[condition]][,3])
  overview_tab_within <- rbind(overview_tab_within, table_within_low[[condition]][,3])
}
colnames(overview_tab_between) <- rownames(table_between_low[[condition]])
colnames(overview_tab_within) <- rownames(table_within_low[[condition]])
#overview_tab <- cbind(overview_tab_between, overview_tab_within)
rownames(overview_tab_between) <- c(30, 40, 50, 62, 75, 87, 100, 112, 125, 137, 150, 160, 175, 187, 200, 212, 225, 237, 250)

pheatmap::pheatmap(overview_tab_between, display_numbers = TRUE, cluster_rows = FALSE, cluster_cols = FALSE, angle_col = c("90"), row_names_side = c("left"))
setHook("grid.newpage", NULL, "replace")
grid::grid.text("xlabel example", y=-0.07, gp=grid::gpar(fontsize=10))
grid::grid.text("ylabel example", x=-0.07, rot=90, gp=grid::gpar(fontsize=10))

```


### Pairwise signal plots
This section presents pairwise plots for cross-correlation coefficients that are 1 SD lower than the mean cross-correlation (i.e., threshold).
...
heatmap_high <- corrplot::corrplot(pmax(fullmatrix_high, t(fullmatrix_high), na.rm = TRUE), 
                                   ...
                                   tl.col = "black")
rm(fullmatrix_high, heatmap_high)
```


#### ActiGraph
```{r Heatmap high sampling frequency experiment AG, echo=FALSE, fig.width=14, fig.height=14}
...
heatmap_high_AG <- corrplot::corrplot(pmax(matrix_high_AG, t(matrix_high_AG), na.rm = TRUE), 
                                      ...
                                      col.lim = c(0,1), addCoef.col = 'black', tl.srt=45, diag = TRUE, 
                                      tl.col = "black")
rm(matrix_high_AG, heatmap_high_AG)
```

#### Axivity
```{r Heatmap high sampling frequency experiment Ax, echo=FALSE, fig.width=14, fig.height=14}
...
heatmap_high_Ax <- corrplot::corrplot(pmax(matrix_high_Ax, t(matrix_high_Ax), na.rm = TRUE), 
                                      ...
                                      col.lim = c(0,1), addCoef.col = 'black', tl.srt=45, diag = TRUE, 
                                      tl.col = "black")
rm(matrix_high_Ax, heatmap_high_Ax)
```

#### GENEActiv
```{r Heatmap high sampling frequency experiment GA, echo=FALSE, fig.width=14, fig.height=14}
...
heatmap_high_GA <- corrplot::corrplot(pmax(matrix_high_GA, t(matrix_high_GA), na.rm = TRUE), 
                                      ...
                                      col.lim = c(0,1), addCoef.col = 'black', tl.srt=45, diag = TRUE, 
                                      tl.col = "black")
rm(matrix_high_GA, heatmap_high_GA)
```

### Heatmap per shaker condition
```{r Heatmap high shaking frequency experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=15}
fullmatrices_high <- correlation_matrices_perShakerCondition$high

heatmaps_high <- list()
for (condition in 1:length(fullmatrices_high)){
  heatmaps_high[[condition]] <- corrplot::corrplot(pmax(fullmatrices_high[[condition]]$correlationMatrix,
                          t(fullmatrices_high[[condition]]$correlationMatrix), na.rm = TRUE), 
                            #title = paste0("shaker condition: ", names(fullmatrices_high)[condition]),
                            type = 'lower', method = 'color', order = "alphabet", 
                            number.cex = 1.25, cl.cex = 2, tl.cex = 2,
...
rm(fullmatrices_high, heatmaps_high)
```
...
avg_crosscor_high_between <- list()
avg_crosscor_high_within <- list()

for (condition in 1:length(correlation_matrices_perShakerCondition$high)){
  avg_crosscor_high <- dataframeShapeComparisonBeween(correlation_matrices_perShakerCondition$high[[condition]], specifications_high) 
  avg_crosscor_high_between[[condition]] <- avg_crosscor_high[avg_crosscor_high$same_brand == "0",]
  avg_crosscor_high_within[[condition]] <- avg_crosscor_high[avg_crosscor_high$same_brand == "1",]
}
rm(avg_crosscor_high)
names(avg_crosscor_high_between) <- names(correlation_matrices_perShakerCondition$high)
names(avg_crosscor_high_within) <- names(correlation_matrices_perShakerCondition$high)
```

#### Between brands
```{r Between brand comparison high sampling frequency experiment, echo=FALSE}
table_between_high <- list()
for(condition in 1:length(avg_crosscor_high_between)){
  df_correlations_high <- avg_crosscor_high_between[[condition]]
  df_correlations_high$correlation <- as.numeric(df_correlations_high$correlation)
  avg_cor <- tapply(df_correlations_high$correlation, df_correlations_high$brands, summary)
  
  tab <- rbind(avg_cor$AxivityActigraph, avg_cor$GENEActivActigraph, avg_cor$GENEActivAxivity)
  tab <- tab[,-1]
  tab <- tab[,-5]
  SD <- tapply(df_correlations_high$correlation, df_correlations_high$brands, sd)
  SD <- c(SD[2], SD[4], SD[5])
  tab <- cbind(tab, SD)
  table_between_high[[condition]] <- tab
}
rm(avg_cor, tab, SD)
names(table_between_high) <- names(correlation_matrices_perShakerCondition$low)
```

```{r Print between comparison tables high frequency experiment, echo = FALSE}
for(condition in 1:length(table_between_high)){
  print(paste0("Shaker condition: ", names(table_between_high)[condition]))
  print(table_between_high[[condition]])
}
```

#### Within brands
```{r Within brand comparison high sampling frequency experiment, echo=FALSE}
table_within_high <- list()
for(condition in 1:length(avg_crosscor_high_within)){
  df_correlations_high <- avg_crosscor_high_within[[condition]]
  df_correlations_high$correlation <- as.numeric(df_correlations_high$correlation)
  avg_cor <- tapply(df_correlations_high$correlation, df_correlations_high$brands, summary)
  
  tab <- rbind(avg_cor$ActigraphActigraph, avg_cor$AxivityAxivity, avg_cor$GENEActivGENEActiv)
  tab <- tab[,-1]
  tab <- tab[,-5]
  SD <- tapply(df_correlations_high$correlation, df_correlations_high$brands, sd)
  SD <- c(SD[1], SD[3], SD[6])
  tab <- cbind(tab, SD)
  table_within_high[[condition]] <- tab
}
rm(avg_cor, tab, SD)
names(table_within_high) <- names(correlation_matrices_perShakerCondition$high)
```

```{r Print within comparison tables high frequency experiment, echo = FALSE}
for(condition in 1:length(table_within_high)){
  print(paste0("Shaker condition: ", names(table_within_high)[condition]))
  print(table_within_high[[condition]])
}
```

#### Heatmap table average cross-correlation coefficients per shaker condition
```{r Heatmap table high sampling freqency}
overview_tab_between <- data.frame()
overview_tab_within <- data.frame()

for(condition in 1:length(table_between_high)){
  overview_tab_between <- rbind(overview_tab_between, table_between_high[[condition]][,3])
  overview_tab_within <- rbind(overview_tab_within, table_within_high[[condition]][,3])
}
colnames(overview_tab_between) <- rownames(table_between_high[[condition]])
colnames(overview_tab_within) <- rownames(table_within_high[[condition]])
#overview_tab <- cbind(overview_tab_between, overview_tab_within)
rownames(overview_tab_between) <- c(30, 40, 50, 62, 75, 87, 100, 112, 125, 137, 150, 160, 175, 187, 200, 212, 225, 237, 250)
