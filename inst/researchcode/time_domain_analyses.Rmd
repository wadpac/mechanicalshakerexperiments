...
```{r Heatmap low sampling frequency experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=14}
...
for(condition in 1:length(table_between_low)){
  ...
  overview_tab_within <- rbind(overview_tab_within, table_between_low[[condition]][,3])
}
colnames(overview_tab_between) <- rownames(table_between_low[[condition]])
colnames(overview_tab_within) <- rownames(table_between_low[[condition]])
overview_tab <- cbind(overview_tab_between, overview_tab_within)
rownames(overview_tab) <- c(30, 40, 50, 62, 75, 87, 100, 112, 125, 137, 150, 160, 175, 187, 200, 212, 225, 237, 250)

pheatmap::pheatmap(overview_tab, display_numbers = TRUE, cluster_rows = FALSE, cluster_cols = FALSE, angle_col = c("90"), row_names_side = c("left"))
...
### Heatmap per shaker condition
```{r Heatmap high shaking frequency experiment per shaker condition, echo=FALSE, fig.width=14, fig.height=14}
...
for (condition in 1:length(fullmatrices_high)){
  heatmaps_high[[condition]] <- corrplot::corrplot(pmax(fullmatrices_high[[condition]]$correlationMatrix,
                          t(fullmatrices_high[[condition]]$correlationMatrix), na.rm = TRUE), 
                            ...
                            type = 'lower', method = 'color', order = "alphabet", 
                            number.cex = 1.25, cl.cex = 2, tl.cex = 2,
                            col.lim = c(-1,1), addCoef.col = 'black', tl.srt = 45, 
                            diag = TRUE, tl.col = "black")
}
