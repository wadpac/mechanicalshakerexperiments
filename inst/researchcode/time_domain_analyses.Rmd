  ...
  for (x in 1:nrow(correlationMatrix)){
    for (y in 1:ncol(correlationMatrix)){
      if (correlationMatrix[x,y] < 1 & !is.na(correlationMatrix[x,y])) {
        #maybe add condition: if correlation < .3? Now also pairwise plots are printed for high correlations.
        if(correlationMatrix[x,y] <= threshold){
          counter = counter + 1
...
### Heatmap
```{r Heatmap high frequency experiment, echo=FALSE, , out.extra='angle=90', fig.width=14, fig.height=14}
...
```{r Summary statistics high frequency experiment, echo=FALSE}
# cross-correlations
...
```{r High Frequency Experiment: Pairwise plots, include=FALSE}
...
### High sampling frequency experiment
```{r Descriptives high samping frequency experiment, echo=FALSE}
library(dplyr)
